/*
  ...
  Spring Term 2025 
  Pose Classification with a touch of Audio
*/
...
function preload() {
  ...
  //load all audio files dynamically using a loop
  for (let i = 1; i < 5; i++) {
    let audio = loadSound(`audio_files/bird_${i}.mp3`)
...
function draw() {
  ...
  //trigger audio files depending on the detected pose
  playAudio();
}

function playAudio() {
  //check if the poseLabel has changed
  if (poseLabel !== lastPoseLabel) {
    //stop all sounds to prevent overlap
    stopAllSounds();

    //play the corresponding audio file based on the new poseLabel
    if (poseLabel === '1') {
      audioFiles[0].play(); // Plays bird_1.mp3
    } else if (poseLabel === '2') {
      audioFiles[1].play(); // Plays bird_2.mp3
    } else if (poseLabel === '3') {
      audioFiles[2].play(); // Plays bird_3.mp3
    } else if (poseLabel === '4') {
      audioFiles[3].play(); // Plays bird_4.mp3
    }

    //update the lastPoseLabel to the current poseLabel
